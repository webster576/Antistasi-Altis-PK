...
if (str (_display) != "no display") then
{
	...
	_ChildControl  ctrlSetTooltip format ["Cost: %1 P",[vehSDKHeliMi8] call A3A_fnc_vehiclePrice];
	...
	_ChildControl  ctrlSetTooltip format ["Cost: %1 P",[vehSDKPlaneSu25] call A3A_fnc_vehiclePrice];
	_childControl ctrlSetText format ["%1",getText (configFile >> "CfgVehicles" >> vehSDKPlaneSu25 >> "displayName")];
	_ChildControl = _display displayCtrl 106;
	_ChildControl  ctrlSetTooltip format ["Cost: %1 P",[vehSDKPlaneMig29S] call A3A_fnc_vehiclePrice];
